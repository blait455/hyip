...
class PaymentController extends Controller
{
    ...
    public function depositConfirm(Request $request)
    {
        ...
        if ($data->gateway_id == 101) {
            ...
            $title = $gatewayData->name;
            $boompay['amount'] = $data->amount;
            $boompay['value1'] = $gatewayData->val1;
            $boompay['value2'] = $gatewayData->val2;
            $boompay['track'] = $track;
            return view('user.payment.boompay', compact('boompay', 'gnl', 'currency', 'title'));
            ...
            $bcoin = $data['btc_amo'];
            $title = "Deposit via  ".$method->name;
            $qrurl = "<img src=\"https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=ethereum:$wallet&choe=UTF-8\" title='' style='width:300px;' />";
            return view('user.payment.coinpayeth', compact('bcoin', 'wallet', 'qrurl', 'title'));
        }
